...
with st.container(border=True):
    ...
    ultimo_parte = df_resultados[df_resultados['layer'] == fila_datos['layer']].sort_values(by='FECHA', ascending=False).iloc[0] if not df_resultados[df_resultados['layer'] == fila_datos['layer']].empty else fila_datos
    ...
    with st.form("form_ruta_pro"):
        ...
        with col_e1:
            ...
            idx_est = opciones_estado.index(val_estado_actual) if val_estado_actual in opciones_estado else 0
        ...
        val_pasadas = ca2.number_input("Pasadas", value=safe_float(ultimo_parte.get('PASADAS', fila_datos.get('PASADAS'))), step=1.0, format="%.0f")
        
        # Campo bloqueado, solo visual
        ca3.text_input("Total Km", value="Auto-calc al guardar", disabled=True)
        
        val_recurso = ca4.text_input("Recurso", value=safe_str(ultimo_parte.get('RECURSO', fila_datos.get('RECURSO'))))
        
        st.markdown("##### 🕒 Tiempo (Cálculo Automático)")
        cb1, cb2, cb3, cb4 = st.columns(4)
        
        val_fecha = cb1.date_input("Fecha", value=parsear_fecha(ultimo_parte.get('FECHA', fila_datos.get('FECHA'))))
        val_hinicio = cb2.time_input("Hora Inicio", value=parsear_hora(ultimo_parte.get('H.INICIO', fila_datos.get('H.INICIO'))))
        val_hfin = cb3.time_input("Hora Fin", value=parsear_hora(ultimo_parte.get('H.FIN', fila_datos.get('H.FIN'))))
        ...
        val_personal = cc1.text_input("Personal", value=safe_str(ultimo_parte.get('PERSONAL', fila_datos.get('PERSONAL'))))
        val_fundente = cc2.text_input("Fundente", value=safe_str(ultimo_parte.get('FUNDENTE', fila_datos.get('FUNDENTE'))))

        st.markdown("<br>", unsafe_allow_html=True)
        submit_btn = st.form_submit_button("➕ Calcular y Añadir al Registro Compartido", type="primary")
        ...
        if submit_btn:
            ...
            # 2. CÁLCULO TIEMPO
            dt_inicio = datetime.combine(datetime.today().date(), val_hinicio)
            dt_fin = datetime.combine(datetime.today().date(), val_hfin)
            ...
            # Formatear strings para GUARDAR
            ...
            nueva_fila = {
                ...
                'RUTA': int(fila_datos['RUTA']), # Asegurar tipo
                'TRAMO': fila_datos['TRAMO'],
                'RECORRIDO': fila_datos['RECORRIDO'],
                'ESTADO': val_estado,
                'HIELO': val_hielo,
                'ACTUACION': val_actuacion,
                'DISTANCIA': round(val_distancia, 2),
                'PASADAS': int(val_pasadas), # Asegurar tipo
            ...
            # 🚨 ¡MODIFICACIÓN CLAVE! Escritura en Google Sheets
            conn = st.connection("gsheets", type=GSheetsConnection)
            
            # Convierte la nueva fila a una lista de listas para añadir al final del Sheet
            data_to_add = [list(nueva_fila.values())]
            
            try:
                # La función append_data añade la nueva fila al final del Google Sheet
                conn.append(spreadsheet=GOOGLE_SHEET_URL, worksheet=TABLA_TRABAJO, data=data_to_add)
                st.success(f"✅ Guardado compartido: {fila_datos['layer']} | {minutos_totales:.0f} min | {calculo_total_km:.2f} km")
                
                # Invalida el caché y recarga la aplicación para mostrar el nuevo dato a todos
                st.cache_data.clear()
                st.rerun()

            except Exception as e:
                st.error(f"❌ No se pudo guardar en Google Sheets. Revisa la configuración de la conexión y las credenciales.")
                st.error(f"Detalle del error: {e}")

# --- TABLA COMPARTIDA Y DESCARGA ---
st.subheader("📋 Tabla de Partes (Datos Compartidos)")

# El DataFrame ya contiene los datos compartidos actualizados
if not df_resultados.empty:
    ...
    with col_d1:
        st.download_button(
            label="💾 Descargar CSV",
            data=csv_bytes,
            file_name="parte_rutas_compartido.csv",
            mime="text/csv",
        )
    with col_d2:
        # Se elimina la opción "Limpiar Tabla" para evitar que un usuario borre el registro de todos.
        st.warning("Para limpiar el registro, borra los datos directamente en Google Sheets.")
else:
    st.info("Utiliza el formulario de arriba para añadir tramos. Los datos se cargarán aquí al guardar.")

# Al final de la aplicación, recarga los datos si ha pasado el TTL
# (Esto ayuda a los usuarios que ya la tienen abierta a ver las actualizaciones)
cargar_datos_compartidos()